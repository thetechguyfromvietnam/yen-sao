...
const cartText = cart.length > 0
  ? cart
      .map(i =>
        `${i.qty} x ${i.name} (${currency(i.price)} mỗi ${i.unit}) = ${currency(i.price * i.qty)}`
      )
      .join("\n")
  : "Giỏ hàng trống.";
  ...
  return (
    <div className={`${theme.brand.bg} text-stone-800 min-h-screen`}> 
      ...
      <header className="sticky top-0 z-30 backdrop-blur supports-[backdrop-filter]:bg-white/70 bg-white/60 border-b border-stone-200">
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between">
          ...
          <nav className="hidden md:flex gap-6 text-sm">
            <a href="#san-pham" className="hover:opacity-80">Sản phẩm</a>
            <a href="#uu-diem" className="hover:opacity-80">Ưu điểm</a>
            <a href="#quy-trinh" className="hover:opacity-80">Quy trình</a>
            <a href="#dat-hang" className="hover:opacity-80">Đặt hàng</a>
            <a href="#lien-he" className="hover:opacity-80">Liên hệ</a>
          </nav>
      ...
      <section id="top" className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-4 py-16 md:py-24 flex flex-col md:flex-row items-center gap-10">
          ...
          <motion.div initial={{opacity:0,scale:1}} whileInView={{opacity:1,scale:1}} viewport={{once:true}} transition={{duration:0.6, delay:0.1}}  className="w-full md:w-1/2">
            <div className={`${theme.radius} ${theme.shadow} overflow-hidden ${theme.brand.surface}`}>
              <div className="grid grid-cols-3 gap-10 p-4 sm:p-6 md:p-8">
          ...
          {products.map((p, idx)=> (
            <div key={idx} className={`${theme.radius} ${theme.shadow} ${theme.brand.surface} p-4 flex flex-col`}>
              ...
              <div className="mt-3 flex items-center gap-2">
                <button onClick={()=> setQty(q=> ({...q,[idx]:(q[idx]||0)-1>0 ? (q[idx]-1) : 0}))} className="px-3 py-2 border rounded-lg">-</button>
                <span className="w-10 text-center">{qty[idx]||0}</span>
                <button onClick={()=> setQty(q=> ({...q,[idx]:(q[idx]||0)+1}))} className="px-3 py-2 border rounded-lg">+</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="dat-hang" className="mx-auto max-w-7xl px-4 py-16">
        <div className={`${theme.radius} ${theme.shadow} ${theme.brand.surface} p-6 grid md:grid-cols-2 gap-8`}>
          <div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16">
        <div className={`${theme.radius} ${theme.shadow} ${theme.brand.surface} p-6`}> 
          <h2 className="text-2xl md:text-3xl font-semibold" style={{color: theme.brand.primary}}>Khách hàng nói gì</h2>
          <div className="mt-4 grid md:grid-cols-3 gap-4 text-sm">
            {["Yến sợi to, chưng lên không bị bở.", "Giao nhanh. Đóng gói sạch sẽ để làm quà.", "Mua cho mẹ bầu rất yên tâm."].map((t,i)=> (
              <figure key={i} className="p-4 bg-stone-50 rounded-xl border">
                <div className="flex items-center gap-1 mb-1">{Array.from({length:5}).map((_,k)=> <Star key={k} className="h-4 w-4" style={{color: theme.brand.accent}}/>)}</div>
                <blockquote>“{t}”</blockquote>
                <figcaption className="mt-2 text-stone-500">— Khách {i+1}</figcaption>
              </figure>
            ))}
          </div>
        </div>
      ...
      <section id="lien-he" className="mx-auto max-w-7xl px-4 py-16">
        <div className={`${theme.radius} ${theme.shadow} ${theme.brand.surface} p-6 grid md:grid-cols-2 gap-6`}>
          <div>
